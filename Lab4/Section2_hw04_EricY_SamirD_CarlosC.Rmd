...
Let's now observe the ACF and PACF to understand more about the series.
...
The gradual decline at a reasonably steady pace suggests an AR model with $p \ge 1$.
...
```{r}
model <- Arima(train.time.series$x, order = c(2,1,1), seasonal = list(order = c(2, 1, 2), period = 12), method = 'ML')
```
...
Our model appears to do an excellent job fitting the data in-sample.
