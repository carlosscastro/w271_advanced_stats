...
```{r}
#Load Data
public.opinion = read.csv("public_opinion.csv")
...
#Data Overview
public.opinion$gender_2 <- public.opinion$gender - 1
public.opinion$age <- 2017 - public.opinion$birthyr
#public.opinion$democrat <- as.numeric(public.opinion$party == 1)
public.opinion$independent <- as.numeric(public.opinion$party == 2)
public.opinion$republican <- as.numeric(public.opinion$party == 3)
...
# Distribution of Age
ggplot(public.opinion, aes(x = age)) +
...
ggplot(public.opinion, aes(factor(sanders_preference), age)) +
  geom_boxplot(aes(fill = factor(sanders_preference))) + 
  geom_jitter() +
  ggtitle("Sanders Preference by Age") + 
  theme(plot.title = element_text(lineheight=1, face="bold")) 

ddply(public.opinion,~sanders_preference,summarise,median = median(age),mean=mean(age),sd=sd(age))
...
# Gender by Age
ggplot(public.opinion, aes(factor(gender_2), age)) +
  geom_boxplot(aes(fill = factor(gender_2))) + 
  geom_jitter() +
  ggtitle("Gender by Age") + 
  theme(plot.title = element_text(lineheight=1, face="bold")) 

ddply(public.opinion,~gender_2,summarise,median = median(age),mean=mean(age),sd=sd(age))
...
ggplot(public.opinion, aes(factor(party), age)) +
  geom_boxplot(aes(fill = factor(party))) + 
  geom_jitter() +
  ggtitle("Party Affiliation by Age") + 
  theme(plot.title = element_text(lineheight=1, face="bold")) 

ddply(public.opinion,~party,summarise,median = median(age),mean=mean(age),sd=sd(age))
...
# Race_White by Age
ggplot(public.opinion, aes(factor(race_white), age)) +
  geom_boxplot(aes(fill = factor(race_white))) + 
  geom_jitter() +
  ggtitle("Race_white by Age") + 
  theme(plot.title = element_text(lineheight=1, face="bold")) 

ddply(public.opinion,~race_white,summarise,median = median(age),mean=mean(age),sd=sd(age))
#Whites have a higher median age of 53, and non whites are 43 

ddply(public.opinion,~race_white,summarise,mean=mean(gender_2))
#55% males for non whites, 51% males for whites

ddply(public.opinion[complete.cases(public.opinion),],~race_white,summarise,mean=mean(sanders_preference))
...
#Section 1a.
model.final <- glm(sanders_preference ~ age + gender_2 + race_white + independent + republican + age:race_white, data = public.opinion)
summary(model.final)

odds_age <- 1/exp(model.final$coefficients[2]*10)
odds_age

odds_white<- exp(model.final$coefficients[4])
odds_white

odds_ind <- exp(model.final$coefficients[5])
odds_ind

odds_rep <- exp(model.final$coefficients[6])
odds_rep

#Model Overview
##The model looks at the likelihood of supporting bernie sanders based on 5 explanatory variables. The five explantory variables are age as of 2017, gender (male vs. female), race_white (non white vs. white), independent voter (baseline democrat), republican (baseline democrat).  
...
#Age Variable
#Age has been chosen because that is the main effect we are trying to investigate.  The Age by sander preference bix plot shows how age is ditribution by preference for sanders.  From this we see that sanders supporters skew younger with a median age of 46, compared to a median age of 52 for clinton supports.  This distribution reinforces that we should include age in our model.  
...
complete.po <- public.opinion[complete.cases(public.opinion),]
...
ddply(complete.po,~party,summarise,mean=mean(sanders_preference))
...
ddply(complete.po,~gender,summarise,mean=mean(sanders_preference))
