...
## Limits, outliers and missing values:

We'll use Hmisc package throughout this section.
```{r}
library(Hmisc)
```
...
```{r}
describe(df$Class.Year)
```

```{r}
describe(df$Years.Since.Graduation)
```

### Marital.Status Variable

```{r}
describe(df$Marital.Status)
```
...
```{r}
describe(df$Next.Degree)
```

### AttendenceEvent Variable

```{r}
describe(df$AttendenceEvent)
```

### FY12Giving Variable

```{r}
describe(df$FY12Giving)
```
...
### FY16Givingcategory Variable

```{r}
describe(df$FY16GivingCat)
```
...
mod.multinomial <- multinom(data = df, formula = FY16GivingCat ~ Marital.Status + Major + Next.Degree + AttendenceEvent + Years.Since.Graduation + FY12Giving + FY13Giving + FY14Giving + FY15Giving)
...
mod.multinomial.simple <- multinom(data = df, formula = FY16GivingCat ~ Marital.Status + AttendenceEvent + Years.Since.Graduation + FY14Giving + FY15Giving)

summary(mod.multinomial.simple)

```

```{r}
Anova(mod.multinomial.simple)
```
...
mod.ordinal = clm(data = df, formula = FY16GivingCat ~ Marital.Status + AttendenceEvent + Years.Since.Graduation + FY14Giving + FY15Giving)
summary(mod.ordinal)
```
...
```{r}
library(package = ordinal)
mod.ordinal.simpler = clm(data = df, formula = FY16GivingCat ~ AttendenceEvent + Years.Since.Graduation + FY14Giving + FY15Giving)
summary(mod.ordinal.simpler)
```

## Ordinal Non-Proportional
