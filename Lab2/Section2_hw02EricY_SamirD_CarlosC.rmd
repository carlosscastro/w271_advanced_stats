...
### Multinomial Model
After adding some engineered features we were able to reduce the model AIC to 1168 and reduce the features from 9 to 5.  Reducing the number of features should help with not overfitting the model on a relatively small sample set.  We removed marital status because of small sample sizes for the Widow and Divorced groups.  The addition of log(meandonation+1) and gaveLastYear provide prior donation history and are highly significant in the model based on the Anova test.  The confusion matrix shows a greatly improved overall acuracy with at 67.6% with a 95% confidence between 61.4% and 73.3%.  We also see an improvement in the sensitivity for the highest doner group.  It is important to be able to identify individuals in this group since they account for ~77% of donations in 2016. 
...
### Ordinal Model
Our final model was an oridinal model.  The ordinal model improved slightly on overall accuracy with our test set scoring a 68.8%, however this was not significantly different from our prior multinomial model.  Where the ordinal did improve was the sensitivity/specificity scores for the highest doner groups and the non doner group.  Along with overall accuracy our final evaluation focused correctly identifying the top group because of the large portion of donations they accounted in additon to correctly identifying the zero dollar doners who account for a largest category of doners.  The ordinal model also maintain the parsimony of the previous model, which as noted previously should help against overfitting on a small training/test set.
